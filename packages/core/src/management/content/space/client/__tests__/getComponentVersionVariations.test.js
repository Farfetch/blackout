import { getSpaceComponentVersionVariations } from '..';
import client from '../../../../../helpers/client';
import fixtures from '../__fixtures__/getComponentVersionVariations.fixtures';
import moxios from 'moxios';

describe('getComponentVersionVariations', () => {
  const spaceCode = 'emails';
  const componentCode = 'button';
  const versionId = '1.0';
  const expectedConfig = {
    baseURL: 'https://api.blackandwhite-ff.com',
    headers: {
      'Content-Type': 'application/json',
    },
  };
  const spy = jest.spyOn(client, 'get');
  const endpoint = '/content/v1/spaces';

  beforeEach(() => {
    moxios.install(client);
    jest.clearAllMocks();
  });

  afterEach(() => moxios.uninstall(client));

  it('should handle a client request successfully', async () => {
    const response = {};

    fixtures.success({ spaceCode, componentCode, versionId, response });

    expect.assertions(2);

    await expect(
      getSpaceComponentVersionVariations(spaceCode, componentCode, versionId),
    ).resolves.toBe(response);

    expect(spy).toHaveBeenCalledWith(
      `${endpoint}/${spaceCode}/components/${componentCode}/componentversions/${versionId}/variations`,
      expectedConfig,
    );
  });

  it('should receive a client request error', async () => {
    fixtures.failure({ spaceCode, componentCode, versionId });

    expect.assertions(2);

    await expect(
      getSpaceComponentVersionVariations(spaceCode, componentCode, versionId),
    ).rejects.toMatchSnapshot();

    expect(spy).toHaveBeenCalledWith(
      `${endpoint}/${spaceCode}/components/${componentCode}/componentversions/${versionId}/variations`,
      expectedConfig,
    );
  });
});
