...
export const entitiesMapper = {
  [actionTypes.GET_ORDERS_SUCCESS]: (state, action) => {
    const { splitByMerchantOrderCode } = action.meta;
    ...
    for (const order in orders) {
      if (orders.hasOwnProperty(order)) {
        Object.values(orders[order].byMerchant).forEach(orderByMerchant => {
          if (splitByMerchantOrderCode) {
            Object.values(orderByMerchant).forEach(merchantCode => {
              const merchantOrderCode = merchantCode.merchantOrderCode;
              merchantsFromOrders[
                orderByMerchant[merchantOrderCode].merchant.id
              ] = orderByMerchant[merchantOrderCode].merchant;
              orderByMerchant[merchantOrderCode].merchant =
                orderByMerchant[merchantOrderCode].merchant.id;
            });
          } else {
