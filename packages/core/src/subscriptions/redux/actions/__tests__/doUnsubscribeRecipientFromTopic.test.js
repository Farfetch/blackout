...
import { doUnsubscribeRecipientFromTopic } from '..';
...
describe('Subscriptions redux actions', () => {
  ...
  describe('doUnsubscribeRecipientFromTopic() action creator', () => {
    const deleteRecipientFromTopic = jest.fn();
    const action = doUnsubscribeRecipientFromTopic(deleteRecipientFromTopic);
    ...
    it('Should create the correct actions when the unsubscribe recipient from subscription topic request fails', async () => {
      ...
      try {
        await store.dispatch(
          action(subscriptionId, topicId, recipientId, meta),
        );
      } catch (error) {
    ...
    it('Should create the correct actions when the unsubscribe recipient from subscription topic request is successful', async () => {
      ...
      await store.dispatch(action(subscriptionId, topicId, recipientId, meta));
