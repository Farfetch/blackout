 ...
 *
 * @param {number | string} order - The apperance order for the image sources.
 ...
 * areas.
 */
export default (legacyImages, { productImgQueryParam } = {}) => {
  if (!legacyImages) {
    return;
  }
  ...
  if (!isEmpty(legacyImages)) {
    ...
    const groupedByOrder = groupBy(
      legacyImages,
      image => image.order || image.displayOrder,
    );

    return Object.entries(groupedByOrder).map(([order, imagesSources]) =>
      generateSourcesByOrder(order, imagesSources, {
        productImgQueryParam,
      }),
    );
  }
