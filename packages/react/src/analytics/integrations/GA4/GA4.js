...
class GA4 extends integrations.Integration {
    ...
    try {
      ...
      pageViewCommandList.push([
        ...
        'page_view',
        {
          ...
          path_clean: contextLocation.pathname,
        },
      ]);
  ...
  /**
   * Post-process the event by adding additional parameters.
   * Attaches the unique event ID generated by the analytics package and ensures the page_path and page_clean
   * is present on non page view events (necessary since we're not registering page views with the `config` command anymore).
   *
   * @param {Array} command - The command being processed.
   * @param {object} data - Event data provided by analytics.
   *
   * @returns {Array} The modified command.
   */
  ...
  internalLoadScript(options) {
    ...
    const code =
      ...
      "    gtag('config', \"" +
      this.measurementId +
      `" ,{ debugMode: ${debugMode}, send_page_view: ${this.enableAutomaticPageViews} });\n`;
