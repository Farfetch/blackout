...
import Castle, { castleEvents } from '../Castle/CastleV1';

utils.logger.error = jest.fn();

const appId = 123456;

const createInstanceAndLoad = (options = { appId }, loadData) => {
  const castleInstance = Castle.createInstance(options, loadData);

  // Mimic the onload event from the script
  castleInstance.scriptOnload();
...
describe('Castle integration', () => {
  ...
  describe('Castle instance', () => {
    ...
    it('Should return a Castle instance from createInstance', () => {
      instance = createInstanceAndLoad();

      expect(instance).toBeInstanceOf(Castle);
    });

    it('Should create the castle script and append it to the DOM', () => {
    ...
    it('Should track a page when the initialization is resolved', () => {
      ...
      expect(windowCastleMock).toBeCalledTimes(1);
    });

    it('Should ignore tracking of custom events', () => {
      const windowCastleMock = jest.spyOn(window, '_castle');
