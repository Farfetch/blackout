 ...
 */
export const areUserReturnsFetched = (state: StoreState) => {
  const returns = getUserReturnsFromReducer(state?.users as UsersState);

  return !!returns?.result || (!!returns?.error && !returns?.isLoading);
};
