...
/**
 * Constant that represent the default object returned by content api without content
 */
const defaultContentReturn = {
  number: 0,
  totalItems: 0,
  totalPages: 0,
  entries: [],
...
/**
 * Function that returns the default section which would be returned by content api
 * containing content generated by Content Tool
 */
const createDefaultSectionSubsectionReturn = (components: ComponentType[]) => [
  {
    type: 'list',
    components: [
      {
 ...
 * pages without duplicates. This implementation only works with content from new Content Tool
 *
 * @example
 * ```
...
export const getBestRankedCommercePageUsingMergeStrategy = (
      ...
      component.components?.forEach(component => {
        ...
        component.components?.forEach(component => {
          if (component.customType) {
            ...
            if (
              !mergedComponents.some(
                mergedComponent => mergedComponent.name === component.name,
              )
            ) {
              mergedComponents.push(component);
            }
  ...
  if (!firstEntry) {
    return defaultContentReturn;
  }

  delete firstEntry?.ranking;
  ...
  return {
    ...
    entries: [
      {
        ...firstEntry,
        components: createDefaultSectionSubsectionReturn(mergedComponents),
      },
    ],
  };
};
