/**
 * Addresses actions.
 *
 * @module addresses/actions
 * @category Addresses
 * @subcategory Actions
 */

export { default as createAddressFactory } from './createAddressFactory';
export { default as fetchAddressFactory } from './fetchAddressFactory';
export { default as fetchAddressesFactory } from './fetchAddressesFactory';
export { default as fetchAddressSchemaFactory } from './fetchAddressSchemaFactory';
export { default as fetchDefaultContactAddressFactory } from './fetchDefaultContactAddressFactory';
export { default as fetchPredictionDetailsFactory } from './fetchPredictionDetailsFactory';
export { default as fetchPredictionsFactory } from './fetchPredictionsFactory';
export { default as removeAddressFactory } from './removeAddressFactory';
export { default as removeDefaultContactAddressFactory } from './removeDefaultContactAddressFactory';
export { default as setDefaultBillingAddressFactory } from './setDefaultBillingAddressFactory';
export { default as setDefaultContactAddressFactory } from './setDefaultContactAddressFactory';
export { default as setDefaultShippingAddressFactory } from './setDefaultShippingAddressFactory';
export { default as updateAddressFactory } from './updateAddressFactory';
