 ...
 * the full coverage needed for `defaults.js` file. For that, we need to specify
 * with the comment above that the environment is different from `jsdom` (node).
 * This comment only works if we specify the environment for the whole file, so we
